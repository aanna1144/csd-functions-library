...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║  HATHITRUST SMOKE TEST — catches upstream API drift                        ║
# ║                                                                            ║
# ║  Runs enrich_hathitrust() against a small set of known, previously-        ║
# ║  verified titles and checks the results still match. Meant to be run       ║
# ║  periodically (or before trusting the function for a big job) so a         ║
# ║  future change to HathiTrust's API surfaces as an immediate test failure   ║
# ║  rather than silently corrupting output years from now.                    ║
# ║                                                                            ║
# ║  The default case (OCLC 785, "Foundations of the Metaphysics of Morals")   ║
# ║  was confirmed live against the API during this function's development     ║
# ║  (see project documentation) - a historical record whose rights status     ║
# ║  is not expected to change. Add institution-specific known cases via       ║
# ║  `extra_cases` (data frame with columns: oclc, expected_status, label).    ║
# ║                                                                            ║
# ║  Usage: hathitrust_smoke_test()                                            ║
# ╚════════════════════════════════════════════════════════════════════════════╝

hathitrust_smoke_test <- function(extra_cases = NULL) {
  cases <- data.frame(
    oclc            = c("785"),
    expected_status = c("Limited (search-only)"),
    label           = c("Foundations of the Metaphysics of Morals (1969)"),
    stringsAsFactors = FALSE
  )

  if (!is.null(extra_cases)) {
    cases <- rbind(cases, extra_cases)
  }

  result <- enrich_hathitrust(cases, id_col = "oclc", id_type = "oclc")
  result$passed <- result$`HathiTrust Rights Status` == result$expected_status

  if (all(result$passed)) {
    message("Smoke test PASSED (", nrow(result), "/", nrow(result),
            " known cases matched expected values).")
  } else {
    warning(sum(!result$passed), " of ", nrow(result),
            " smoke test case(s) FAILED -- HathiTrust's API may have changed shape or behavior. ",
            "Review before trusting enrich_hathitrust() output.", call. = FALSE)
  }

  result[, c("label", "oclc", "expected_status", "HathiTrust Rights Status", "passed")]
}